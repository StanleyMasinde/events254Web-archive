...
export default {
  ...
  methods: {
    async handleSubmit () {
      try {
        if (this.editing) {
          await this.$axios.put(`/events/${this.$route.params.event}/tickets/${this.currentTicket.id}`, this.ticket)
        } else {
          await this.$axios.post(`/events/${this.$route.params.event}/tickets`, this.ticket)
        }
